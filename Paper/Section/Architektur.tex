...
\begin{itemize}
	\item \textsl{Speech Recording:} aufnehmen und streamen der Nutzereingabe an die Cloud.
	\item \textsl{Speech Playback:} Abspielen eines Streams, welcher von der Cloud erzeugt wurde.
	\item \textsl{Hotword Detection:} Die Mobile App soll nicht fortlaufend die Eingabe des Nutzers aufnehmen und streamen, denn diese könnte die Privatsphäre des Nutzer beeinträchtigen. Die App soll nur aufnehmen und streamen, wenn der Nutzer das will. Somit kommt die sogenannte "Hotword Detection" zum Einsatz. Diese belauscht den Nutzer durchgängig, jedoch nur lokal auf dem Endgerät und ohne Daten via Internetverbindung weiter zu senden. Eine Hotword Detection benötigt kaum Ressourcen, da die Erkennung nur eines einzigen Signalwortes benötigt. Sobald der Nutzer das definierte Signalwort sagt, wird das Aufnehmen und Streaming an die Cloud gestartet.
\end{itemize}
...
\begin{itemize}
	\item \textsl{Sprachverarbeitung:} Das Modul Speech Processing beinhaltet die  rechenintensiven Funktionen der Sprachverarbeitung. Folgende Teilgebiete der Sprachverarbeitung können für den Sprachassistenten verwendet werden:
	\begin{itemize}
		...
		\item \textsl{Language Translation:} Hierbei geht es um die Übersetzung von Text in eine andere Sprache. Somit kann ein Nutzer mit anderen Personen, die eine unterschiedliche Sprache sprechen, kommunizieren. Des Weiteren kann es sein, dass eine App für den Sprachassistenten nicht in allen Sprachen zur Verfügung steht. Wird die Sprache eines Nutzers nicht unterstützt, so kann die Übersetzung genutzt werden, dass dieser die App nutzen kann.
		\item \textsl{Speaker Identifikation:} Die Sprecherdidentifikation kann als Authentifizierungsmethode des Sprachassistenten dienen. Durch die Sprachinformation in einem Sprachsignal eines Nutzers kann dieser identifiziert werden. Hiermit kann geprüft werden, ob ein Nutzer die Berechtigungen hat, diesen Sprachassistenten zu nutzen.
	\end{itemize}
